...
func (s *Server) registerTableTools() {
	s.impl.AddTool(mcp.NewTool("memory_create_table_note",
		...
		mcp.WithString("attachment", mcp.Description("PREFERRED: vault-relative path of a CSV you ALREADY uploaded — via the HTTP upload endpoint (your MCP base URL plus /upload) or memory_upload_resource. The note references it, no re-upload. Cheapest path: POST the bytes over HTTP, then pass the returned `path` here.")),
